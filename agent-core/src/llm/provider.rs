    ...
    match config.provider.to_lowercase().as_str() {
        "openai" | "deepseek" | "custom" => {
            let base_url = config.base_url.clone().or_else(|| {
                Some(match config.provider.to_lowercase().as_str() {
            ...
            Ok(Box::new(OpenAIProvider::new(&api_key, &config.model, Some(&base_url))))
        }
        "anthropic" => {
            let base_url = config.base_url.as_deref().unwrap_or("https://api.anthropic.com/v1");
            Ok(Box::new(AnthropicProvider::new(&api_key, &config.model, Some(base_url))))
        }
        "ollama" => {
