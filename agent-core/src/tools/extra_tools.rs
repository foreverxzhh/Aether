...
impl Tool for CronJob {
    ...
    fn description(&self) -> &str { "管理定时任务（创建、列表、删除）" }
    ...
    async fn call(&self, args: Value) -> Result<String, AetherError> {
        ...
        match action {
            "list" => Ok(json!({"jobs":[],"note":"Cron 调度需要外部服务(系统cron/任务队列)触发"}).to_string()),
            "create" => {
                let name = args.get("name").and_then(|v| v.as_str()).unwrap_or("unnamed");
                let sched = args.get("schedule").and_then(|v| v.as_str()).unwrap_or("* * * * *");
...
impl Tool for ImageGenerate {
    ...
    fn description(&self) -> &str { "生成 AI 图像（通过 API 调用）" }
    fn parameters(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn call(&self, args: Value) -> Result<String, AetherError> {
        ...
        let provider = args.get("provider").and_then(|v| v.as_str()).unwrap_or("dalle");
        Ok(json!({
...
impl Tool for HomeAssistant {
    fn name(&self) -> &str { "ha_list_entities" }
    fn description(&self) -> &str { "列出 HomeAssistant 中的所有实体" }
    fn parameters(&self) -> Value { json!({"type":"object","properties":{}}) }
    async fn call(&self, _args: Value) -> Result<String, AetherError> {
        Ok(json!({"entities":[],"note":"HomeAssistant 集成需要配置 HASS_TOKEN 和 HASS_URL"}).to_string())
    }
}
