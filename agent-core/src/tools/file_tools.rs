...
impl Tool for ReadFile {
    ...
    async fn call(&self, args: Value) -> Result<String, AetherError> {
        let path = secure_path(args.get("path").and_then(|v| v.as_str()).ok_or(
...
impl Tool for Patch {
    ...
    async fn call(&self, args: Value) -> Result<String, AetherError> {
        ...
        std::fs::write(&path, &result)
            .map_err(|e| AetherError::ToolExecutionError(format!("写入失败: {}", e)))?;
        let diff = TextDiff::from_lines(&content, &result).unified_diff().to_string();
...
impl Tool for SearchFiles {
    ...
    async fn call(&self, args: Value) -> Result<String, AetherError> {
        ...
        for entry in glob::glob(&pattern_str).map_err(|e| AetherError::ToolExecutionError(e.to_string()))?.flatten().take(50) {
            if let Some(ct) = content_filter {
                if let Ok(text) = std::fs::read_to_string(&entry) {
                    if text.contains(ct) { results.push(entry.display().to_string()); }
                }
            } else { results.push(entry.display().to_string()); }
        }
...
mod tests {
    ...
    async fn test_security_reject_absolute() {
        #[cfg(unix)] assert!(secure_path("/etc/passwd").is_err());
        #[cfg(windows)] assert!(secure_path("C:\\Windows\\system32").is_err());
    }
    #[tokio::test]
