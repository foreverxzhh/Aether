...
impl Tool for DockerTerminal {
    fn name(&self) -> &str { "docker_terminal" }
    fn description(&self) -> &str { "在 Docker 容器中执行命令（需要 Docker daemon）" }
    ...
    async fn call(&self, args: Value) -> Result<String, AetherError> {
        let container = args.get("container").and_then(|v| v.as_str()).ok_or(
            AetherError::ToolInvalidArgs("缺少 container 参数".into()))?;
        let cmd = args.get("command").and_then(|v| v.as_str()).unwrap_or("echo hello");
...
impl Tool for SshTerminal {
    fn name(&self) -> &str { "ssh_terminal" }
    fn description(&self) -> &str { "通过 SSH 在远程主机上执行命令" }
    fn parameters(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn call(&self, args: Value) -> Result<String, AetherError> {
        let host = args.get("host").and_then(|v| v.as_str()).ok_or(
            AetherError::ToolInvalidArgs("缺少 host 参数".into()))?;
        let cmd = args.get("command").and_then(|v| v.as_str()).unwrap_or("echo hello");
...
impl Tool for ExecuteCode {
    ...
    fn description(&self) -> &str { "在隔离进程中执行代码片段（Python/JavaScript），限制资源" }
    fn parameters(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn call(&self, args: Value) -> Result<String, AetherError> {
        let lang = args.get("language").and_then(|v| v.as_str()).ok_or(
            AetherError::ToolInvalidArgs("缺少 language 参数".into()))?;
        let code = args.get("code").and_then(|v| v.as_str()).ok_or(
            AetherError::ToolInvalidArgs("缺少 code 参数".into()))?;
        ...
        let output = tokio::time::timeout(
            ...
            tokio::process::Command::new(interpreter).arg(arg).arg(code).output()
        ).await
